...
function GenreCard({ genre, count, tracks, playlistId, playlistName }) {
  const navigate = useNavigate();
  const [hovered, setHovered] = useState(false);
  const accent = genreAccent(genre);
  const previewImgs = tracks.slice(0, 4).map((t) => t.albumImg).filter(Boolean);

  const handleClick = () => {
    navigate(`/dj-playlists/${playlistId}/genre/${encodeURIComponent(genre)}`, {
      state: { tracks, genre, playlistName },
    });
  };
...
export default function DJPlaylistGenresPage() {
  ...
  useEffect(() => {
    ...
    setError(null);
    axios
      .get(`/playlist/djPlaylist/${playlistId}`, {
      ...
      {loading ? (
        ...
        <Box sx={{ mb: 5 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, mb: 0.5 }}>
            <TuneRoundedIcon sx={{ color: '#4F8EF7', fontSize: 26 }} />
            <Typography sx={{ fontSize: '2rem', fontWeight: 800, color: '#fff', letterSpacing: '-0.035em', lineHeight: 1.1 }}>
              {data.playlistName}
            </Typography>
          </Box>
          <Typography sx={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.9375rem' }}>
            {data.totalTracks} tracks · {data.genres.length} genres
          </Typography>
        </Box>
      ) : null}
      ...
      >
        {loading
          ? Array(6).fill(0).map((_, i) => (
              <Box key={i} sx={{ borderRadius: 2.5, background: '#141414', border: '1px solid rgba(255,255,255,0.06)', borderLeft: '3px solid rgba(255,255,255,0.1)', p: 2.5 }}>
          ...
          : data?.genres.map(({ genre, count, tracks }) => (
              <GenreCard
                key={genre}
                genre={genre}
                count={count}
